...
# Настройка логгера для подробного логирования ошибок вебхука
logger = logging.getLogger("yoomoney_webhook")
...
async def yoomoney_webhook(webhook_data: YooMoneyWebhookSchema):
    try:
        ...
        payment_data = {
            ...
            'payed_at': (
                webhook_data.object.captured_at.isoformat()
                if webhook_data.object.captured_at
                else payment_date.isoformat()
            ),
            'balance': amount_kopeks if webhook_data.object.paid else 0,
            'payment_id': payment_id
        }
        logger.info(f"Received webhook: {webhook_data}")
        logger.info(f"Parsed payment_data: {payment_data}")
        await app.state.database.update_payment_from_webhook(payment_data)
        logger.info("Payment data successfully updated in database.")
        return JSONResponse(
            {
                'status': 'success'
            },
        ...
        logger.error(
            f"KeyError or ValueError in webhook handler: {error}",
            exc_info=True
        )
        raise HTTPException(
            status_code=400,
        ...
        logger.error(
            f"Unhandled exception in webhook handler: {error}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
